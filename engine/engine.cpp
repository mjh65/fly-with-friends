...
std::shared_ptr<IEngine> IEngine::New(ISimData* sim, const std::string& dir)
{
    return std::make_shared<Engine>(sim, dir);
}

Engine::Engine(ISimData* sim, const std::string& dir)
:   workDirPath(dir),
    simulation(sim),
    findDelay(0),
    frameNumber(0)
...
{
    FWF_LOG_INFO(infoLogging,"stop session server");
...
float Engine::DoFlightLoop()
{
    ...
    if (activeOthers > 0)
    {
        ...
        simulation->ActiveOtherAircraft(active);
        ...
        simulation->ActiveOtherAircraft(0);
    }
