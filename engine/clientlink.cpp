...
bool ClientLink::AsyncDisconnectFromSession()
{
    ...
    {
        FWF_LOG_INFO(infoLogging,"sending leave notification");
        if (datagramLog)
        {
...
void ClientLink::IncomingDatagram(AddressedDatagram dgin)
{
    ...
    FWF_LOG_INFO(infoLogging,"received datagram - %08x, %d, %d", sn, cmd, pl);
...
void ClientLink::IncomingWorldState(const char * payload, unsigned int length)
{
    ...
    if (pl < 2)
    {
        FWF_LOG_WARN("ignoring short peer notification datagram");
        return;
    }
...
{
    FWF_LOG_VERBOSE(verboseLogging,"async flight loop started");
    while (1)
    {
        {
            std::unique_lock<std::mutex> lock(flightLoopCVGuard);
        ...
        if ((sessionState != JOINING) && ((lt - lastRcvTime) > MEMBERSHIP_TIMEOUT_MS))
        {
            std::lock_guard<std::mutex> lock(guard);
            sessionState = GONE;
            break;
        ...
        FWF_LOG_VERBOSE(verboseLogging, "creating aircraft position reporting dgram");
    ...
    FWF_LOG_VERBOSE(verboseLogging,"async flight loop completed");
    return true;
}
