...
{
    LOG_VERBOSE(verboseLogging,"%s: queue datagram for sending", name.c_str());
    {
...
{
    LOG_VERBOSE(verboseLogging,"%s: signalling async sender thread", name.c_str());
...
void UdpSocketLocal::OpenAndBindSocket()
{
    ...
    {
        LOG_ERROR("%s: UDP socket bind failed", name.c_str());
        throw - 1;
    }
#endif
    ...
    {
        LOG_WARN("%s: UDP socket getsockname failed", name.c_str());
#ifdef TARGET_WINDOWS
        closesocket(udpSocket);
#else
        close(udpSocket);
    ...
    else
    {
        ...
        LOG_INFO(infoLogging, "%s: assigned local port number %u", name.c_str(), port);
    }
}
...
int UdpSocketLocal::WaitReceive()
{
    ...
    LOG_VERBOSE(verboseLogging,"%s: received datagram from %s : %d len=%d", name.c_str(), a.GetQuad().c_str(), ntohs(from.sin_port), recsize);
#ifndef NDEBUG
    ...
    *(pktDebugBuffer+(2*recsize)) = 0;
    LOG_DEBUG(debugLogging,"%s: %s", name.c_str(), pktDebugBuffer);
#endif
...
int UdpSocketLocal::SendQueued()
{
    ...
    while (1)
    {
        ...
        LOG_VERBOSE(verboseLogging,"%s: sending datagram to %s : %d", name.c_str(), next->Address().GetQuad().c_str(), next->Address().GetPort());
        sockaddr_in target;
