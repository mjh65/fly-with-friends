...
JoiningDialog::JoiningDialog(UIManager * m)
:    Dialog(m, 320, 170, "Fly-With-Friends - Join Session")
{
}

JoiningDialog::~JoiningDialog()
{
}

void JoiningDialog::CreateDialogWidgets()
{
    int x = left;
    int y = top;
    ...
    serverCaption = XPCreateWidget(x + 10, y - 80, x + 90, y - 102,
        ...
        xpWidgetClass_Caption);
    serverEdit = XPCreateWidget(x + 100, y - 80, x + 220, y - 102,
        1, "51.6.219.247", 0, dialog,
        xpWidgetClass_TextField);
    XPSetWidgetProperty(serverEdit, xpProperty_TextFieldType, xpTextEntryField);
    XPSetWidgetProperty(serverEdit, xpProperty_Enabled, 1);
    ...
    // Join activation button
    joinButton = XPCreateWidget(x + 210, y - 140, x + 310, y - 162,
        1, " Join session", 0, dialog,
        xpWidgetClass_Button);
    XPSetWidgetProperty(joinButton, xpProperty_ButtonType, xpPushButton);

}

int JoiningDialog::HandleMessage(XPWidgetMessage msg, XPWidgetID id, intptr_t p1, intptr_t p2)
{
    if ((msg == xpMsg_PushButtonPressed) && (p1 == (intptr_t)joinButton))
    {
        ...
        passcode[63] = 0;
        XPGetWidgetDescriptor(serverEdit, addr, 64);
        addr[63] = 0;
...
void JoiningDialog::SetName(const char *a)
{
    XPSetWidgetDescriptor(nameEdit, a);
}

void JoiningDialog::SetCallsign(const char *a)
{
    XPSetWidgetDescriptor(callsignEdit, a);
}

void JoiningDialog::SetServerAddress(const char *a)
{
    XPSetWidgetDescriptor(serverEdit, a);
}

void JoiningDialog::SetServerPort(const char *a)
{
    XPSetWidgetDescriptor(portEdit, a);
}

void JoiningDialog::SetPasscode(const char *a)
{
